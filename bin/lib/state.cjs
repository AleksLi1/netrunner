...
/**
 * Write a WAITING.json signal file when Netrunner hits a decision point.
 * External watchers (fswatch, polling, orchestrators) can detect this.
 * File is written to .planning/WAITING.json (or .nr/WAITING.json if .nr exists).
 * Fixes #1034.
 */
function cmdSignalWaiting(cwd, type, question, options, phase, raw) {
  const nrDir = fs.existsSync(path.join(cwd, '.nr')) ? path.join(cwd, '.nr') : path.join(cwd, '.planning');
  const waitingPath = path.join(nrDir, 'WAITING.json');
